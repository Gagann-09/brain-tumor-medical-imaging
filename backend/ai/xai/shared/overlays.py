...
class OverlayGenerator:
    ...
    def render_2d(layers: OverlayLayers, alpha: float = 0.5) -> np.ndarray:
        ...
        if layers.mask is not None:
            ...
            overlaid = np.where(layers.mask[..., None] > 0, cv2.addWeighted(overlaid, 0.7, mask_rgb, 0.3, 0), overlaid)
    ...
    def render_2d_with_metadata(layers: OverlayLayers, alpha: float = 0.5) -> np.ndarray:
        ...
        for k, v in layers.metadata.items():
            text = f"{k}: {v}"
            cv2.putText(img_array, text, (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            y_offset += 20

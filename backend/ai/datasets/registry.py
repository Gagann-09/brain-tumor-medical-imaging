...
class RegistryEntry(BaseModel):
    ...
    execution_profile: str = Field(
        ..., description="Execution profile (e.g. development, research)"
    )
        ...
        for entry in self._entries.values():
            if entry.dataset_identifier in identifiers:
                raise MedicalImagingError(
                    f"Duplicate registry identifier found: {entry.dataset_identifier}"
                )
            ...
            if not Path(entry.split_manifest).exists():
                logger.warning(
                    f"Split manifest {entry.split_manifest} for {entry.dataset_identifier} does not exist."
                )

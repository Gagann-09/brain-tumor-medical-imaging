...
from ai.inference.context import InferenceContext, InferenceResult, StageDiagnostics, StageStatus
from ai.inference.manifest import PipelineManifest
from ai.inference.policy import InferencePolicy
...
class InferenceOrchestrator:
    def __init__(
        ...
        self.stages = [
            stage_classes[name](name=name) for name in manifest.stages if name in stage_classes
        ]
        ...
        for i, stage in enumerate(self.stages):
            ...
            self.event_bus.publish(
                Event(
                    ...
                    {"stage": stage.name, "status": stage_diag.status.value},
                )
            )

...
class ClinicalReportGenerationStage(BaseStage):
    def execute(self, context: InferenceContext) -> Tuple[InferenceContext, StageDiagnostics]:
        ...
        report = {
            ...
            "findings": {
                "tumor_present": True if context.classification_results else False,
                "primary_classification": context.classification_results.get("tumor_type", "unknown") if context.classification_results else None,
                "confidence_score": context.classification_results.get("confidence", 0.0) if context.classification_results else 0.0,
                "roi_bbox": context.roi_information.get("bbox", None) if context.roi_information else None
            },
        ...
        return new_context, StageDiagnostics(self.name, StageStatus.SUCCESS, "Structured report generated.")

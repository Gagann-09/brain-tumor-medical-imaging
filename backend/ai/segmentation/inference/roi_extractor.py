...
class ROIStrategy(ABC):
    ...
    @abstractmethod
    def extract(self, image: torch.Tensor, mask: torch.Tensor, padding: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class MaskCropStrategy(BoundingBoxStrategy):
    """
    Crops the image to the bounding box and masks out the background (sets non-tumor pixels to zero).
    """
    def extract(self, image: torch.Tensor, mask: torch.Tensor, padding: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:

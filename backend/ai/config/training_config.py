...
class HardwareConfig(BaseModel):
    ...
    device_ids: list[int] = Field(
        default_factory=lambda: [0], description="List of device IDs to use"
    )
...
class SchedulerConfig(BaseModel):
    ...
    kwargs: dict[str, Any] = Field(
        default_factory=dict, description="Additional scheduler arguments"
    )
...
class AugmentationConfig(BaseModel):
    train_transforms: list[dict[str, Any]] = Field(
        default_factory=list, description="Training transforms configuration"
    )
    val_transforms: list[dict[str, Any]] = Field(
        default_factory=list, description="Validation transforms configuration"
    )
...
class DatasetConfig(BaseModel):
    registry_id: str = Field(
        default="brats20_dev_v1", description="Unique identifier for the dataset in the registry"
    )

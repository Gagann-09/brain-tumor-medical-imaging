...
class InferenceConfig(BaseModel):
    ...
    confidence_threshold: float = Field(default=0.5, ge=0.0, le=1.0, description="Minimum confidence for positive prediction")
    mixed_precision: bool = Field(default=False, description="Use mixed precision inference if available")
    num_workers: int = Field(default=0, ge=0, description="Number of data loading workers")
    return_attention_maps: bool = Field(default=False, description="Whether to return attention/saliency maps")

...
class TrainingConfig(BaseModel):
    ...
    optimizer_kwargs: dict[str, Any] = Field(default_factory=dict, description="Additional optimizer arguments")
    loss_function: str = Field(default="cross_entropy", description="Primary loss function")
    early_stopping_patience: int | None = Field(default=10, description="Epochs to wait for improvement before stopping")
    gradient_clip_val: float | None = Field(default=None, description="Gradient clipping maximum value")
    mixed_precision: bool = Field(default=True, description="Use mixed precision training if available")

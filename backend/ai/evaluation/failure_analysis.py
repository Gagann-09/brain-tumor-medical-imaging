...
class FailureAnalyzerCallback(Callback):
    ...
    def analyze_and_export(self):
        ...
        if "pred_sum" in df.columns and "gt_sum" in df.columns:
            ...
            analysis["largest_over_segmentation"] = df.nlargest(5, "volume_diff").to_dict(orient="records")
            analysis["largest_under_segmentation"] = df.nsmallest(5, "volume_diff").to_dict(orient="records")
        ...
        with open(md_path, "w") as f:
            ...
            for category, cases in analysis.items():
                ...
                for c in cases:
                    ...
                    metrics_str = ", ".join([f"{k}: {v:.4f}" if isinstance(v, float) else f"{k}: {v}" for k, v in c.items()])
                    f.write(f"| {c_id} | {metrics_str} |\n")
                f.write("\n")
                
        print(f"Failure analysis exported to {self.output_dir}")

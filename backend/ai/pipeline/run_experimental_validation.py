...
# Platform Core
from ai.config.training_config import ExperimentConfig, HardwareConfig, ModelConfig, DatasetConfig, OptimizerConfig
from ai.training.manager import TrainingManager
from ai.training.strategy import StandardTrainingStrategy, GANTrainingStrategy, AlternatingUpdatePolicy
from ai.training.components import MultiOptimizerManager, AdversarialLossManager
...
def create_model_and_strategy(model_name: str, config: ExperimentConfig, hardware_manager):
    if model_name == "UNetBaseline":
        model = UNetBaseline(in_channels=config.model.in_channels, out_channels=config.model.out_channels)
        optimizer = torch.optim.Adam(model.parameters(), lr=config.optimizer.learning_rate)
        # Assuming a dummy mixed precision for now
        class DummyMP:
            def autocast(self): return torch.amp.autocast(device_type="cpu", dtype=torch.float32)
            def scale_loss(self, loss): return loss
            def step_optimizer(self, opt): opt.step()
        ...
        def g_bce(fake_preds, **kwargs):
            if isinstance(fake_preds, (list, tuple)): fake_preds = fake_preds[-1]
        ...
        def d_bce(real_preds, fake_preds, **kwargs):
            if isinstance(real_preds, (list, tuple)): real_preds = real_preds[-1]
            if isinstance(fake_preds, (list, tuple)): fake_preds = fake_preds[-1]
            real = torch.nn.functional.binary_cross_entropy_with_logits(real_preds, torch.ones_like(real_preds))
            fake = torch.nn.functional.binary_cross_entropy_with_logits(fake_preds, torch.zeros_like(fake_preds))
        ...
        opt_d = torch.optim.Adam(model.discriminator.parameters(), lr=config.optimizer.learning_rate)
        ...
        strategy = GANTrainingStrategy(multi_opt, DummyMP(), hardware_manager, None, AlternatingUpdatePolicy())
        return model, strategy

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--profile", type=str, choices=["mock", "development", "research", "production"], default="mock")
    ...
    if args.profile == "mock":
        ...
        print(f"Profile {args.profile} expects real data. Using mock for demo.")
        epochs = 1
        batch_size = 2
        studies = generate_mock_dataset(num_samples=10)
    ...
    train_loader = torch.utils.data.DataLoader(PyTorchDatasetAdapter(TrainingDataset(train_studies)), batch_size=batch_size)
    val_loader = torch.utils.data.DataLoader(PyTorchDatasetAdapter(ValidationDataset(val_studies)), batch_size=batch_size)
    test_loader = torch.utils.data.DataLoader(PyTorchDatasetAdapter(ValidationDataset(test_studies)), batch_size=1)
    ...
    mgr_unet = TrainingManager(cfg_unet, unet_model, unet_strategy, train_loader, val_loader, hardware)
    ...
    mgr_armt = TrainingManager(cfg_armt, armt_model, armt_strategy, train_loader, val_loader, hardware)
    ...
    record, unet_res, armt_res = run_comparison(unet_model, armt_model, test_loader, str(registry_dir), repro_meta=repro_meta)
    ...
    print("--- Evaluating Promotion ---")
    promo_manager.mark_as_candidate("ARMTGANModel", "v1.0")
    success = promo_manager.evaluate_promotion("ARMTGANModel", "v1.0", unet_res, armt_res, record.benchmark_id)
    
    if success:
        print("ARMT-GAN successfully passed validation metrics! State updated to VALIDATED.")
    else:
        print(f"ARMT-GAN failed validation. Reason: {promo_manager.records['ARMTGANModel_v1.0'].promotion_reason}")
    ...
    print(f"Model card saved to {registry_dir}")

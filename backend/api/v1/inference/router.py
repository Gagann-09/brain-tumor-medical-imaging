...
async def submit_inference(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid study_metadata JSON") from None
...
async def get_inference_status(
    job_id: str, service: InferenceService = Depends(get_inference_service)
):
...
async def get_inference_artifacts(
    ...
    return {
        "explainability": job.final_result.get("explainability", {}),
        "report": job.final_result.get("report", {}),
    }


@router.delete("/{job_id}")
async def cancel_inference(job_id: str, service: InferenceService = Depends(get_inference_service)):

...
from storage.security import NoOpScanner, ClamAVScanner, VirusScanner
...
class UploadService:
    ...
    def __init__(self, db: AsyncSession = None) -> None:
        ...
        self.scanner: VirusScanner = NoOpScanner() if self.settings.ENVIRONMENT == "development" else ClamAVScanner()

    async def process_upload(self, file: UploadFile, patient_id: uuid.UUID, user_id: uuid.UUID) -> dict:
        ...
        if not is_safe:
            raise HTTPException(status_code=400, detail="File failed security scan (malware detected).")
        ...
        is_dev_image = False
        if self.settings.ALLOW_IMAGE_UPLOADS_FOR_DEV and kind and kind.mime in ["image/png", "image/jpeg"]:
            is_dev_image = True
        ...
        if not is_dev_image:
            ...
            try:
                ...
                if not (kind and kind.extension == "gz"): # Very loose check, refine later
                     raise HTTPException(status_code=400, detail="Invalid file format. Only DICOM/NIfTI allowed in production.")

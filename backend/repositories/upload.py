...
from schemas.upload import UploadRecord, UploadState
...
class UploadRepository:
    ...
    def get_by_upload_id(self, upload_id: str) -> Optional[UploadRecordModel]:
        return self.db.execute(select(UploadRecordModel).where(UploadRecordModel.upload_id == upload_id)).scalars().first()

    def update(self, upload_id: str, update_data: dict) -> Optional[UploadRecordModel]:

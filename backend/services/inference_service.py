...
class InferenceService:
    ...
    def execute_inference_sync(self, job_id: str, retry_attempt: bool = False):
        ...
        self.repo.update(job_id, {"status": JobState.RUNNING.value, "updated_at": datetime.utcnow()})
        ...
        try:
            ...
            for stage_name, diag in result_dict["diagnostics"].items():
                if hasattr(diag["status"], "value"):
                    diag["status"] = diag["status"].value
                    
            # Store only artifact references (handled natively by orchestrator context if designed that way)
            self.repo.update(job_id, {
                "status": status,
    ...
    def cancel_job(self, job_id: str) -> bool:
        db_job = self.repo.get_by_job_id(job_id)
        if not db_job or db_job.status not in [JobState.QUEUED.value, JobState.RUNNING.value, JobState.RETRYING.value]:
            return False

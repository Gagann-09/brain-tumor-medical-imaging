...
from ai.evaluation.robustness.calculators import DatasetLeakageDetector
...
def test_robustness():
    ...
    assert curve_analysis["divergence"]
    ...
    promoted = pm.evaluate_promotion_robust(
        "model_A", "v1", score_with_leakage, ["Excessive Generalization Gap"], "bench_1"
    )
    assert not promoted

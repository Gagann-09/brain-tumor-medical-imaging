...
from ai.training.callbacks import CheckpointCallback, EarlyStoppingCallback, ModelCardCallback, Callback
...
from ai.training.hardware import HardwareManager
...
def parse_args():
    ...
    parser.add_argument("--profile", type=str, default="development", choices=["development", "research"])
    parser.add_argument("--epochs", type=int, default=None, help="Override profile epochs (req for research)")
...
def main():
    ...
    ds_profile = get_profile(DatasetProfileName.DEVELOPMENT if is_dev else DatasetProfileName.RESEARCH)
    ...
    train_ds, val_ds = torch.utils.data.random_split(torch_dataset, [train_size, val_size], generator=torch.Generator().manual_seed(config.seed))
    ...
    class DummyMixedPrecision:
        ...
        def unscale_gradients(self, opt): pass
        def step_optimizer(self, opt): opt.step()
        
    strategy = StandardTrainingStrategy(
        optimizer=optimizer, 
        mixed_precision=DummyMixedPrecision() if is_dev else DummyMixedPrecision(), 
        hardware=hardware,
        event_bus=None
    )
    ...
    manager.register_callback(ModelCardCallback(
        config=config,
    ...
    manager.register_callback(ValidationVisualizationCallback(
        output_dir=f"outputs/{config.logging.experiment_name}",
        num_cases=5,
        seed=config.seed
    ))

    from ai.evaluation.failure_analysis import FailureAnalyzerCallback
    manager.register_callback(FailureAnalyzerCallback(
        output_dir=f"outputs/{config.logging.experiment_name}"
    ))

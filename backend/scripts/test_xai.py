...
from ai.interfaces.features import FeatureProvider, FeatureMap
from ai.xai.shared import ExplainerRegistry, OverlayLayers, OverlayGenerator, ExplainabilityReport, BenchmarkInfo, ExplainabilitySession, XAIBenchmarker
from ai.xai.classification import GradCAM, SaliencyMap
...
class DummyModel(nn.Module, FeatureProvider):
    ...
    def get_intermediate_features(self):
        return {
            "conv": FeatureMap(tensor=self.last_features, metadata={"channels": 16, "resolution": "32x32"})
        }
...
def run_test():
    ...
    heatmap, benchmark = XAIBenchmarker.run(explainer.generate_explanation, input_tensor=input_tensor, target_class=1)
